...
const userController = {
    ...
    activate: async (req,res) => {
        try {
            ...
            // activation success
            res.status(200).json({msg:"Your account is actived successfully. You can sign in."})
        } catch (err) {
            res.status(500).json({msg: err.message})
        }
    },
    signin: async (req,res) => {
        try {
            ...
            if(!isMatch) return res.status(400).json({msg:"This password is not correct."})
    ...
    forgot: async (req,res) => {
        try {
            ...
            // send mail
            const url = `http://localhost:8000/auth/reset-password/${ac_token}`;
    ...
    google: async (req, res) => {
        try {
            ...
            // failed verfication
            if(!email_verified) return res.status(400).json({msg:"Email verifaction Failed."})
